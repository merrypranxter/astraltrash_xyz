...
const positionLogic = `    const heroRect = homeHero.getBoundingClientRect();
    ...
    cssScale = isMobile ? 1 : Math.max(1.8, preferredScale * 1.8);
    ...
    if (h1Element && !isMobile) {
       ...
       leftPos = h1Rect.right + window.scrollX + 15;
    } else {
...
// fix double observer bug
code = code.replace(
  /observer = new MutationObserver\(schedulePlacement\);\s+observer = new MutationObserver\(schedulePlacement\);/,
