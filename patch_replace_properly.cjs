...
const newRenderReport = `function renderReport() {
...
const startIdx = ts.indexOf('function renderReport() {');
const endIdx = ts.indexOf('function renderIntro() {');

if (startIdx !== -1 && endIdx !== -1) {
  ts = ts.substring(0, startIdx) + newRenderReport + '\n\n' + ts.substring(endIdx);
  fs.writeFileSync('src/psychedelic-portal.ts', ts);
  console.log("Replaced renderReport successfully.");
} else {
  console.log("Could not find renderReport or renderIntro");
}
