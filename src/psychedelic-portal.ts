  ...
  return PSY_REPORTS.map((report, index) => `
    <button class="psy-fragment ${index === activeReport ? 'is-active' : ''}" data-report="${index}" type="button">
      <div style="display: flex; flex-direction: column; width: 100%; align-items: center;">
        ...
        <span class="psy-fragment-label" style="font-family: 'Pixelogist', monospace; font-size: clamp(8px, 1.2vw, 16px); letter-spacing: 0.3em; text-transform: uppercase; margin-top: clamp(20px, 9vw, 120px); white-space: nowrap;">${htmlEscape(report.nav)}</span>
      </div>
    </button>
  `).join('');
}
