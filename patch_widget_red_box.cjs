...
const positionLogic = `    const heroRect = homeHero.getBoundingClientRect();
    ...
    // Scale matching the red box proportion (about 2.0 to 2.2 on desktop)
    cssScale = isMobile ? 1.2 : 2.1;
    ...
    if (h1Element && !isMobile) {
       ...
       // Position overlapping the end of the h1 slightly (like the red box)
       leftPos = h1Rect.right + window.scrollX - (W * cssScale * 0.65);
    } else if (subElement && isMobile) {
       leftPos = subElement.getBoundingClientRect().right + window.scrollX - (W * cssScale);
    } else {
       leftPos = heroRect.right + window.scrollX - (W * cssScale);
    }
